...
async def lifespan(_: FastAPI):
    ...
    # Insert seed data and create initial versions
    with SessionLocal() as db:
        # Insert documents with current_version = 1
        db.execute(insert(models.Document).values(id=1, content=DOCUMENT_1, current_version=1))
        db.execute(insert(models.Document).values(id=2, content=DOCUMENT_2, current_version=1))

        # Create Version 1 for each document
        db.execute(insert(models.DocumentVersion).values(
            document_id=1, version_number=1, content=DOCUMENT_1
        ))
        db.execute(insert(models.DocumentVersion).values(
            document_id=2, version_number=1, content=DOCUMENT_2
        ))
